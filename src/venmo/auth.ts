...
import { API_BASE, APP_HEADERS, CLIENT_ID } from "./constants.ts";
import { VenmoApiError, VenmoAuthError } from "./types.ts";
...
export type LoginResult =
	...
	| { kind: "needs_otp"; challenge: OtpChallenge };
...
export async function loginWithPassword(opts: {
	...
	const errCode = (body as { error?: { code?: number } }).error?.code;
	if (errCode === TWO_FACTOR_ERROR_CODE) {
		const otpSecret = res.headers.get("venmo-otp-secret");
		if (!otpSecret) {
			throw new VenmoAuthError(
				"Venmo signaled 2FA but no venmo-otp-secret header was returned",
			);
		}
	...
	if (res.ok) {
		...
		if (!token) {
			throw new VenmoAuthError(
				`Login succeeded (${res.status}) but no access_token. Body: ${bodyText.slice(0, 500)}`,
			);
		}
	...
	if (res.status === 401) {
		throw new VenmoAuthError(
			`Venmo rejected credentials (401). Body: ${bodyText.slice(0, 500)}`,
		);
	}
...
export async function loginWithOtp(opts: {
	...
	try {
		...
		throw new VenmoAuthError(
			`OTP login response was not JSON: ${body.slice(0, 200)}`,
		);
	}
	const token = (parsed as { access_token?: string }).access_token;
	if (!token) {
		throw new VenmoAuthError(
			`OTP login response missing access_token: ${body.slice(0, 200)}`,
		);
	}
	return token;
}
