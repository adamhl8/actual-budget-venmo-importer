import type { ActualTransaction } from "#actual/client.ts"
import type { VenmoStory, VenmoUser } from "#venmo/types.ts"

const requirePayment = (story: VenmoStory) => {
  if (!story.payment) throw new Error(`Story ${story.id} (type=${story.type}) has no payment object`)

  return story.payment
}

const userDisplayName = (user: VenmoUser): string => {
...
const isoToActualDate = (iso: string): string => iso.slice(0, 10)

const signedAmountCents = (story: VenmoStory, meId: string): number => {
...
const payeeNameForStory = (story: VenmoStory, meId: string): string => {
  const payment = requirePayment(story)
  const other = payment.actor.id === meId ? payment.target.user : payment.actor
  return userDisplayName(other)
}

export const storyToTransaction = (story: VenmoStory, meId: string, accountId: string): ActualTransaction => {
  const payment = requirePayment(story)
