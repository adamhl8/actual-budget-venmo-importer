...
const boolFromString = z
	...
	.transform((v) =>
		typeof v === "boolean" ? v : /^(1|true|yes|on)$/i.test(v),
	);
...
export function requireActualConnection(c: Config): ActualConnection {
...
export function requireActualSync(c: Config): ActualSyncConfig {
	const base = requireActualConnection(c);
	if (!c.actual.venmoAccountId) {
		throw new Error(
			"Missing required environment variable: ACTUAL_VENMO_ACCOUNT_ID (run `list-accounts` to find it)",
		);
	}
