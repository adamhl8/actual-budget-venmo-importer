...
const formatValue = (v: unknown): string => {
  if (typeof v === "string") return /[\s"=]/v.test(v) ? JSON.stringify(v) : v
...
const makeLogger = (level: Level, context: Record<string, unknown>): Logger => {
  ...
  const emit = (lvl: Level, msg: string, fields?: Record<string, unknown>): void => {
    ...
    const tail = Object.keys(merged).length > 0 ? ` ${formatFields(merged)}` : ""
...
export const createLogger = (level: Level): Logger => makeLogger(level, {})
